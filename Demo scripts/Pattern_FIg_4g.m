...
% General
Simulation_time = 30000; % In seconds - overall simulation time
Time_step = 0.25; % In seconds - used time step for updating concentrations (lower value gives more accurate results)
Update_delay = 250; % In seconds - frequency of figures update
...
Switch_time = 3000; % In seconds - time when the concentration of Fast Inhibitor changes
C0_I_end = 6.5; % New concentrition for Fast Inhibitor
...
%Reaction kinetics
k0 = 0.005; % 1/s - Supply rate of the componenets from the CSTR into the reaction media
k1 = 1*10^(-3); % 1/mM*s - Autocatalysis rate constant (S + A/GA -> A/GA + A)
k2 = 0.3; % 1/mM*s - Fast inhibition rate constant(A/GA + I -> x)
k3 = 7.5*10^(-3); % 1/s - Slow inhibition rate constant (A/GA -> x)
